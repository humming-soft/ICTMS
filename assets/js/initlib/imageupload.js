...
var checkAndSendAllImages = function(e) {
    e.preventDefault();
	if (checkImageUpload()) {
		sendAll();
	}
...
var checkImageUpload = function(){
	var ok = true;
	$.each($('.description-textarea'), function(a){
        // Added trim function to check the white spaces
	...
	if (!ok) { 
        $("#error_notify span").text("Image descriptions are compulsory.");
        $("#error_notify").show();
	}else{
        $("#error_notify").hide();
    }
...
var disableBeforeSend = function() {
...
$(function () {
	...
	$('.closebutton, #dcs_a_de_pj_ia .close').on('click',function(){
...
$(function () {
    ...
    $('.closebutton, #dcs_a_de_pj_iu .close').on('click',function(){
        if (shouldRefresh) location.reload();
    })
});