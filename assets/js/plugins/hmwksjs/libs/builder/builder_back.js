...
// Modified By "https://github.com/sebint/"
...
/* iframe */
	...
	selectNode:  function(node) {
		...
		//Added by Sebin
		// if($(target).data('highlight') && $(target).data('highlight') == 'none'){ 
		// 	jQuery("#select-box #select-actions").hide();
		// }else{
		// 	jQuery("#select-box #select-actions").show();
		// }
		//Added by Sebin
	},
...
/* iframe highlight */    
    _initHighlight: function() {
						...
						if (event.originalEvent)
						{
							if ((offset.top  < (event.originalEvent.y - halfHeight)) || (offset.left  < (event.originalEvent.x - halfWidth)))
							{
								 if (isIE11){
									 ...
									 //If Condition Added By Sebin
									//if(jQuery(event.target).data("drop") && jQuery(event.target).data("drop")!='none'){
		...
		this.frameBody.on("dblclick", function(event) {
			...
			{
				//If Coonditon Added By Sebin - From here
				//if(jQuery(event.target).data("highlight")!='none'){
				//Until Here	
...
/* drag and drop */
	_initDragdrop : function() {
		...
		$('body').on('mousemove touchmove', function(event) {
			if (self.iconDrag && self.isDragging == true)
			{
				self.iconDrag.css({'right': event.originalEvent.x - 60, 'top':event.originalEvent.y - 30});
...
Hmwks.Gui = {
	...
	save : function () {
		...
		$.post(Hmwks.Config.getUrl(), { data: dataContent,[csrfinfo.csrfName] : csrfinfo.csrfToken }, function (d) {
			if(d.status == 1){
				alert(d.msg);
			}
		...
		function drawComponentsTree(tree)
		{
			...
			for (i in tree)
			{
				...
				if (tree[i].children.length > 0) 
				{
					...
					<label for="id' + j + '" style="background-image:url(assets/js/plugins/hmwksjs/libs/builder/' + node.image + ')"><span>' + node.name + '</span></label>\
